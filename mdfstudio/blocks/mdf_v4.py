...
ASAM MDF version 4 file format module
    ...
    functions
    ---------
    *   MDF4._read : Calculate sampling rate of channel group and set as Channel's attribute
    *   get_fmt_v4 - Fix format as "<f2" if size == 1 and data_type == v4c.DATA_TYPE_REAL_INTEL
    ...
    * ``param`` - row size used for tranposizition, in case of tranposed zipped blockss
    ...
    def _read(self, mapped=False):
        ...
        for gp_index, grp in enumerate(self.groups):
            ...
            # get sampling rate
            timestamps = self.get_master(gp_index)
            if len(timestamps) <= 3:
                continue
            result = 0
            for i in range(100):
                random_t = random.randrange(2, len(timestamps)-1)
                result += timestamps[random_t] - timestamps[random_t - 1]
            result = result / 100
            ...
            for x in range(9):
                for y in range(4):
                    val = pow(2,x) * pow(5,y)
                    if val <= 1000:
                        raster_under.add(1/val)
            raster_under = list(raster_under)
            raster_under.sort()

            raster_over = [1, 1/0.2, 1/0.4, 1/0.5, 1/0.8]
            ...
            if int(result_str.split(".")[0]) > 0:
                for pos in reversed(range(4)):
                    if found:
                        break
                    for raster in raster_over:
                        if round(result, pos) == raster:
                            ch_raster = raster
                            found = True
                            break
            else:
                for pos in reversed(range(11)):
                    if found:
                        break
                    for raster in raster_under:
                        if round(result, pos) == raster:
                            ch_raster = raster
                            found = True
                            break

            if ch_raster is None:
                min_value = 100
                min_raster = temp_raster = round(result, 5)
                raster_all = raster_over + raster_under
                for raster in raster_all:
                    if abs(temp_raster - raster) < min_value:
                        min_value = abs(temp_raster - raster)
                        min_raster = raster
                ch_raster = min_raster

            for _, ch in enumerate(grp.channels):
                ch.sampling_rate = ch_raster
    ...
    def append(self, signals, source_info="Python", common_timebase=False, units=None):
        ...
        source_info : str | mdfstudio.source_utils.Source
        ...
        >>> mdf = MDF4('new.mdf')
        >>> mdf.append([s1, s2, s3], 'created by mdfstudio v4.0.0')
        ...
        >>> sigs = [ch1, ch2]
        >>> mdf2 = MDF4('out.mf4')
        >>> mdf2.append(sigs, 'created by mdfstudio v4.0.0')
    ...
    def extend(self, index, signals):
        ...
        >>> s3 = Signal(samples=s3, timestamps=t, unit='flts', name='Floats')
        >>> mdf = MDF4('new.mdf')
        >>> mdf.append([s1, s2, s3], 'created by mdfstudio v1.1.0')
...
<tool_id>mdfstudio</tool_id>
<tool_vendor>mdfstudio</tool_vendor>
...
#            self.name.unlink()
    ...
    def get(
        ...
        * if the channel index is out of range
        ...
        Examples
        --------
        >>> from mdfstudio import MDF, Signal
        >>> import numpy as np
        >>> t = np.arange(5)
        >>> s = np.ones(5)
