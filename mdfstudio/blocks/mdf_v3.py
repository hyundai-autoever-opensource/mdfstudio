...
""" ASAM MDF version 3 file format module
    ...
    Functions
    ---------
    *   MDF3._read - Calculate sampling rate of channel group and set as Channel's attribute
"""
...
class MDF3(object):
    ...
    * ``trigger`` - *Trigger* object for current group
    ...
    def append(
        ...
        >>> sigs = [ch1, ch2]
        >>> mdf2 = MDF3('out.mdf')
        >>> mdf2.append(sigs, 'created by mdfstudio v1.1.0')
    ...
    def save(self, dst, overwrite=False, compression=0):
        ...
        if not self.header.comment:
            text = f"exported by mdfstudio {__version__}"
            ...
            text = f"{old_history}\n{timestamp}: updated by mdfstudio {__version__}"
            self.header.comment = text
